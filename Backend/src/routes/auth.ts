...
// --- TEMPORARY: demo login (remove before production) ---
const TEMP_DEMO_EMAIL = 'dhwani@gmail.com';
const TEMP_DEMO_PASSWORD = 'dhwani08';
const DEMO_USER_ID = 'demo-user';
const DEMO_USER = { id: DEMO_USER_ID, name: 'Dhwani', email: TEMP_DEMO_EMAIL };
...
router.post('/login', async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    // TEMPORARY: demo login works without DB (remove before production)
    if (emailNorm === TEMP_DEMO_EMAIL && password === TEMP_DEMO_PASSWORD) {
      const secret = JWT_SECRET || 'demo-secret-change-in-production';
      const token = jwt.sign({ userId: DEMO_USER_ID }, secret, { expiresIn: JWT_EXPIRY });
      res.json({ success: true, token, user: DEMO_USER });
      return;
    }
